...
# Load global icon system with fallback - determine PORTX_ROOT first
# Determine PORTX_ROOT using same logic as portx-tools
if [[ -f "C:/App/Git/doc/tools.md" ]]; then
    PORTX_ROOT="C:/App/Git"
elif [[ -n "$PORTX_ROOT" ]]; then
    PORTX_ROOT="$PORTX_ROOT"
elif [[ -n "${BASH_SOURCE[0]}" ]]; then
    PORTX_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
else
    echo "$(icon_error) Error: Cannot locate PORTX installation"
    exit 1
fi

# Load global icon system with fallback
if [[ -f "$PORTX_ROOT/home/portx/scripts/theme.sh" ]]; then
    source "$PORTX_ROOT/home/portx/scripts/theme.sh"
else
    # Fallback icon functions if theme.sh not available
    icon_package() { echo "📦"; }
    icon_directory() { echo "📁"; }
    icon_network() { echo "🌐"; }
    icon_search() { echo "🔍"; }
    icon_statistics() { echo "📊"; }
    icon_success() { echo "✅"; }
    icon_error() { echo "❌"; }
    icon_warning() { echo "⚠️"; }
fi
